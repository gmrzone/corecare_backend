...
def signal_save_placeholder_helper(instance, sender, blur, height, quality):
    ...
    if instance._state.adding:
        ...
        image_file, image_name, file_size, content_type = generate_placeholder(instance.icon, blur, height, quality)
    ...
    if not instance._state.adding:
        ...
        image_file, image_name, file_size, content_type = generate_placeholder(instance.icon, 4, 200, 50)
        instance.placeholder = InMemoryUploadedFile(file=image_file, field_name="placeholder", name=image_name, size=file_size, content_type=content_type, charset='utf-8')
...
def signal_delete_placeholder_helper(instance, sender):
    ...
    if (not obj.icon or obj.icon is not None) and (not obj.placeholder or obj.placeholder is not None):
