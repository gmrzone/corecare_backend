...
def create_order(request):
    ...
    try:
        ...
        service_recommandation = Recommender().get_basic_recommandation(
            create_recommandation_list, max_result=4
        )
        ...
        data = {
            "status": "ok",
            "msg": f"Paymant sucessfull your order with order id {order.receipt} has been created ",
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            ...
            if serializer.is_valid():
                ...
                data = {
                    "status": "ok",
                    "msg": f"Paymant sucessfull your order with order id {receipt} has been created ",
                    "receipt": receipt,
                }
            else:
...
def add_from_recommanded_toCart(request):
    ...
    if service_id and category:
        ...
        if len(get_recommandation_for) > 0:
            rec = recommander.get_detail_recommandation(
                get_recommandation_for, max_result=7
            )
        else:
            rec = []
...
def download_invoice(request, order_id):
    ...
    response["Content-Disposition"] = (
        "attachments; filename=invoice_corecare_" + order_id + ".pdf"
    )
