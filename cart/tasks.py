...
def order_success_mail(order_id, recommanded_ids):
    order = (
        Order.objects.select_related("category", "user", "coupon")
        .prefetch_related("items", "items__service", "coupon__category")
        .get(receipt=order_id)
    )
    preserve_ids = Case(
        *[When(id=id, then=index) for index, id in enumerate(recommanded_ids)]
    )
    recommanded_services = Service.objects.filter(id__in=recommanded_ids).order_by(
        preserve_ids
    )
    subject = f"Order {order_id}"
    body = render_to_string(
        "emails/order_success.html", {"order": order, "services": recommanded_services}
    )
    mail = EmailMessage(
        subject=subject,
        body=body,
        from_email="saiyedafzal0@gmail.com",
        to=[order.user.email],
    )
