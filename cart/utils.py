...
if settings.DEBUG:
    ...
    r = Redis(host=settings.REDIS_HOST, port=settings.REDIS_PORT, db=settings.REDIS_DB, password=settings.REDIS_PASSWORD)
...
class Recommender:
    ...
    def get_detail_recommandation(self, services, max_result=5):
        ...
        if len(suggestion_ids) > 0:
            preserve_ids = Case(*[When(id=id, then=index) for index, id in enumerate(suggestion_ids)])
            suggestion_services = Service.objects.filter(id__in=suggestion_ids).order_by(preserve_ids)
